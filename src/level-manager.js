...
export class Level {
  constructor(description, measures) {
...
class LevelManager {
  ...
  getLevel(index) {
    ...
    const notes = RhythmPatterns.generateNotes(5, 15, 50);
    const measures = RhythmPatterns.generateMeasures([120], [FOUR_FOUR], notes);
  ...
  registerLevels(levels) {
    this.levels = levels.map((level, index) => {
      ...
      const timeSignatures = level[1].map(TimeSignature.parse);
      const tempi = level[2];
      const notes = Note.parseNotes(level[3]);
      const measures = RhythmPatterns.generateMeasures(tempi, timeSignatures, notes);
...
levelManager.registerLevels(LEVELS_DATA);
